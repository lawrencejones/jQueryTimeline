...
(function() {
  var SETTINGS, animate_moments, create_interval_markers, create_moments, create_spine, draw_end_wires, layer_moment_tooltips, make_circle, month_num_to_name, parse_date, produce_wire;
  ...
  create_moments = function(spine) {
    ...
    return infos.hide().delay(400).fadeIn(200, function() {
  ...
  draw_end_wires = function(m, vh) {
    ...
    if (m.is_expanded && (m.horizontal_end_wire == null)) {
      ...
      return m.horizontal_end_wire = produce_wire(m, m.start).delay(200).addClass('horizontal end').css('top', vh).animate({
        width: w
      }, {
        duration: 200,
        complete: function() {
  ...
  animate_moments = function() {
    var b, bottom_room, c, css, e, h, height, i, m, ms, t, vh, vt, _i, _len, _ref, _ref1, _ref2, _ref3;
    ms = SETTINGS.moments;
    for (_i = 0, _len = ms.length; _i < _len; _i++) {
      m = ms[_i];
    ...
    return SETTINGS.spine.animate({
      top: 100 * Math.abs(t) / (b - t) + '%'
    }, {
      duration: 200
    });
  };
