...
window.createTimeline = (args) ->
  ...
  else if not args.container? then console.log "No jQuery Container element."
  else if not args.moments? then createEmptyTimeline(
    args.start, args.end, 'day', args.container, (args.structure ?= {}))
  else createTimelineWithMoments(
    args.start, args.end, 'day', args.container, args.moments, (args.structure ?= {}))
...
window.createEmptyTimeline = (startDate, endDate, interval, jQueryObject, structure) ->
...
# spine object
window.createMomentsAtSpine = (moments, spine) ->
...
window.createMomentAtSpine = (m, spine, utils) ->
...
createAndPlaceMomentDots = (moment,spine) ->
  ...
  (moment.endDot = makeCircle(7,'#E0524E'))
      ...
      .hide()
  moment.duration = createDurationLine(moment,getUtils(spine),'#5BB35C')
