...
(function() {
  var adjustHeights, animateCircleGrowth, animateEndWires, assignMomentTopValues, bindExpandAllToOrigin, buildLabel, center, createAndPlaceMomentDots, createAndPlaceMomentInfo, createAndPlaceMomentStartWire, createDurationLine, createTimelineContainer, createTimelineWithMoments, drawInMarkers, drawTimelineOriginCircle, drawTimelineSpine, formatDate, getContainer, getNextId, getUtils, makeCircle, makeMarker, monthNumToName, parseDate, processExpanded, processTitle, produceIntervals, setContainerData, setPriority, sortMoments, updateMomentInfoCSS;

  window.createTimeline = function(args) {
    var _ref, _ref1;
    if (args.start == null) {
      return console.log("No start date.");
    } else if (args.end == null) {
      return console.log("No end date.");
    } else if (args.container == null) {
      return console.log("No jQuery Container element.");
    } else if (args.moments == null) {
      return createEmptyTimeline(args.start, args.end, 'day', args.container, ((_ref = args.structure) != null ? _ref : args.structure = {}));
    } else {
      return createTimelineWithMoments(args.start, args.end, 'day', args.container, args.moments, ((_ref1 = args.structure) != null ? _ref1 : args.structure = {}));
    }
  };
  ...
  window.createEmptyTimeline = function(startDate, endDate, interval, jQueryObject, structure) {
  ...
  window.createMomentsAtSpine = function(moments, spine) {
    var m, _i, _j, _len, _len1, _results;
    spine.data('moments', moments);
    for (_i = 0, _len = moments.length; _i < _len; _i++) {
      m = moments[_i];
      m = createMomentAtSpine(m, spine, getUtils(spine));
    }
  ...
  window.createMomentAtSpine = function(m, spine, utils) {
    if (utils == null) {
  ...
  assignMomentTopValues = function(moments, spine) {
    ...
    return adjustHeights(moments, spine);
  };

  adjustHeights = function(moments, spine) {
    var adjustForClash, downs, findMaxBottom, findMaxTop, m, processLayers, ups, utils, _fn, _fn1, _i, _j, _k, _l, _len, _len1, _len2, _len3;
  ...
  createAndPlaceMomentDots = function(moment, spine) {
    ...
    (moment.endDot = makeCircle(7, '#E0524E')).delay(1400).css({
      left: 0,
      zIndex: 11
    }).animate({
      left: endLeft
  ...
  processTitle = function(m, infoDiv, structure, utils) {
    ...
    return processExpanded(m, mainTitle, infoDiv, structure, utils, textLine);
  };

  processExpanded = function(m, mainTitle, infoDiv, structure, utils, textLine) {
    var addedContent, content, i, key, link, linkKey, links, _i, _j, _k, _l, _len, _len1, _ref, _ref1, _ref2;
    ...
    if (structure.content.names != null) {
      ...
      for (i = _j = 0, _len = _ref1.length; _j < _len; i = ++_j) {
        ...
        if (m[linkKey] != null) {
          ...
          link.text(structure.content.names[i]).attr('href', m[linkKey]);
          links.push(link);
        }
      }
